...
- 🎨 **Mapa de Desvio Visual Inteligente** — Mapa de calor sobreposto na peça indicando desvios críticos (Vermelho), toleráveis (Amarelo) e ideais (Verde), com posicionamento automático inteligente da legenda (inferior ou superior esquerdo) para evitar sobreposições ao corpo de prova. O quadro de métricas sobreposto foi removido para maior clareza visual, com todos os dados transferidos para o relatório.
- 📈 **Perfil de Variação Suave** — Gráficos de perfil dimensional (0% a 100%) com margens de escala corrigidas. Para cilindros na vista superior (Top), o perfil nominal CAD é traçado como um arco suave utilizando a fórmula analítica de corda circular; na vista lateral (Side) e para prismas, o perfil CAD é plotado como uma linha reta ideal, correspondendo fielmente às projeções geométricas.
- 📊 **Exportação CSV** — Medições completas, retração percentual, desvios CAD, metadados de escala e cotas transversais relativas extraídas a **20%, 50% e 90%** de comprimento e largura para desconsiderar distorções nas extremidades.
- 🖼️ **Imagens Anotadas com Cotas** — Desenho de cotas gráficas de engenharia (linhas de cota, extensão, ticks de 45° e o valor medido em mm) diretamente sobre a imagem final no perímetro do bounding box da peça e marcações nas seções de 20%, 50% e 90%, garantindo saídas autoexplicativas e profissionais.
- 📄 **Relatório HTML Moderno e Sequencial** — Geração de relatórios visuais completos com layout lado a lado (foto em perspectiva à esquerda e resumo metrológico com cards empilhados à direita: dimensões da peça real com destaque de fonte, dimensões nominais CAD e precisão IoU). A visualização das análises de desvio superior (Top) e lateral (Side) agora é feita sequencialmente de forma contínua (sem abas), facilitando a leitura completa e a exportação direta para PDF.
